...
function removeIssueItem(id){
  ...
  const labelSymbolsDOM = document.querySelectorAll(".fa-location-dot");
  ...
  for (let j = 0; j < labelSymbolsDOM.length; j++){
    const symbol = labelSymbolsDOM[j];
    console.log(symbol);
    if (symbol.getAttribute('data-id') === id) {
      labelSymbolsDOM.removeChild(symbol);
      break;
    }
  }
}
...
window.addEventListener('dblclick', (event) => {
  ...
  //   label.classList.add('fa-beat-fade');
  // });
  // label.addEventListener('mouseout', () => {
  //   label.classList.remove('fa-beat-fade');
  // });


  // add label info to issues side panel
  // displayIssueItemsDOM(labelsArray);
  // function displayIssueItemsDOM(labArray) {
  //   labelsArray.forEach((issueLab) => {
  //     console.log(issueLab);
  //     addLabelToIssuesOverlay(issueLab);
  //   });
  // }
...
export { addLabelToModel as a, addLabelToIssuesOverlay as b, camera as c, canvas as d, scene as s, updateIssuesOverlay as u };
