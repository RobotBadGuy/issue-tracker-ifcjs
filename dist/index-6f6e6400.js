...
function zoomToIssue(posX, posY, posZ){
    ...
    console.log(posY);
    console.log(posZ);
    cameraControls.setLookAt(posX, posY, posZ , 0, 10, 0, true);
...
const addLabelToIssuesOverlay = (labelItem, labelImg, currentViewPos) => {
  ...
  let LabelXPos = currentViewPos.x;
  let LabelYPos = currentViewPos.y;
  let LabelZPos = currentViewPos.z;
...
window.addEventListener("dblclick", (event) => {
  ...
  console.log(cameraControls.getPosition.x);

  const currentViewpoint = new CustomVector3(cameraControls.getPosition.x, cameraControls.getPosition.y, cameraControls.getPosition.z, id);
  viewpointArray.push(currentViewpoint);
  ...
  addLabelsToOverlay(labelsArray, imagesArray, currentViewpoint);
...
function addLabelsToOverlay(UpdatedLabelsArray, updatedImagesArray, currentViewPos){
  updateIssuesOverlay();
  let currentView = currentViewPos;
  UpdatedLabelsArray.map((label) => {
    const matchingImg = updatedImagesArray.find((labelImg) => labelImg.dataset.id === label.element.dataset.id);
    addLabelToIssuesOverlay(label, matchingImg, currentView);
  });
}
