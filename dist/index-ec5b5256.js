...
const addLabelToIssuesOverlay = (labelItem, labelImg, camPos) => {
  console.log(camPos);
  const labelItemID = labelItem.element.dataset.id;
  const labelImgSrc = labelImg.src;
  const LabelXPos = camPos.x;
  const LabelYPos = camPos.y;
  const LabelZPos = camPos.z;
...
const camPosZ$1 = (camera.position.z = 5);
const camPosX$1 = (camera.position.x = 5);
const camPosY$1 = (camera.position.y = 5);
camera.lookAt({ x: camPosX$1, y: camPosY$1, z: camPosZ$1 }); //lookAt needs a vector3 (Vector3 {x: 0, y: 0, z: 0}) to focus cam and pos
// camera.lookAt(axes);
...
window.addEventListener("dblclick", (event) => {
  ...
  addLabelsToOverlay(labelsArray, imagesArray, currentCamPos);
...
function addLabelsToOverlay(UpdatedLabelsArray, updatedImagesArray, camView){
  updateIssuesOverlay();
  UpdatedLabelsArray.map((label) => {
    const matchingImg = updatedImagesArray.find((labelImg) => labelImg.dataset.id === label.element.dataset.id);
    addLabelToIssuesOverlay(label, matchingImg, camView);
  });
}
