...
document.querySelector('.issues-item-count');
document.querySelector('.issues-items');
document.querySelector('.issues-total');
...
function removeIssueItem(id){
  const articles = issueItemsOverlayDOM.querySelectorAll('.cart-item');
...
// TO BE COMPLETED
// function removeLabelFromModel(id) {
...
function setupIssuesFunctionality() {
  issueItemsOverlayDOM.addEventListener('click', function(e){
    ...
    }    // displayIssuesItemsCount()
    // displayIssuesTotal()
    // setStorageItem('issues',issues)
  });
}
...
function removeLabelFromScene(id){
  const labelRendererDOM = document.getElementById('labelrendererdom');
  labelRendererDOM.querySelector('div[data-id="' + id + '"]');
  // const updatedArray = labelsArray.filter(item => item.dataset.id !== id);
  // const labelToRemove = scene.querySelector('[data-id="' + id + '"]');

  const CSS2DObjectToRemove = scene.children.find(object => console.log(object.element));
  console.log(CSS2DObjectToRemove);
  scene.children.forEach(child => {
    if (child.element && child.element.dataset.id === dataIdToRemove) ;
  });
  console.log();

  const labelToRemove = scene.getObjectById('labelrendererdom');

  if (labelToRemove){
    scene.remove(labelToRemove);
  }

  // const childDivs = labelRendererDOM.querySelectorAll('div');
  // const newInnerHTML = Array.from(childDivs).filter(div => div.dataset.id !== id).map(div => div.outerHTML).join('');
  // console.log(newInnerHTML);
  // labelRendererDOM.innerHTML = '';
  // labelRendererDOM.innerHTML = newInnerHTML;
  // scene.remove()
// if (nodeToRemove) {
...
export { addLabelToModel as a, addLabelToIssuesOverlay as b, camera as c, canvas as d, removeLabelFromArray as r, scene as s, updateIssuesOverlay as u };
